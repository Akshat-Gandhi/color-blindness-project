...
from .utils2 import Transforms2, Utils2
...
class Core2:
    ...
    def simulate(self, simulated_recolored=False):
        ...
        if simulated_recolored == True:
            img_lms = Utils2.load_lms(self.recolored)
        else:
            img_lms = Utils2.load_lms(self.image)
            
        transform = Transforms2.lms_tritanopia_sim()
        
        # Transforming the LMS Image
        img_sim = np.dot(img_lms, transform)
        
        # Converting back to RGB colorspace
        img_sim = np.uint8(np.dot(img_sim, Transforms2.lms_to_rgb()) * 255)
    ...
    def correct(self):
        ...
        img_rgb = Utils2.load_rgb(self.image)

        transform = Transforms2.correction_matrix()
